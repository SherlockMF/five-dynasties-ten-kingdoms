...
本机存储 `li-jingxun.archive.discoveries.v1`，封装 `{version:1, discoveries:[...]}`；开发模式使用独立 `.dev` 后缀。切换开发 / 正式模式时重建独立页面状态，避免旧模式内容残留。清空会作废尚未完成的文件读取和网络请求，过期结果不能恢复清空前状态；读取期间仍可执行本机清空，且不依赖网络。当前不做账号或跨设备同步。
