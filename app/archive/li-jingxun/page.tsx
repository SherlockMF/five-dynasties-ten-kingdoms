...
export default async function ArchivePage({ searchParams }: { searchParams: Promise<{ archiveDev?: string }> }) {
  ...
  return <ArchiveShell initialView={projectArchive(archiveEntries, archiveSources, [], liJingxunEntityMapping)} allowDev={process.env.NODE_ENV === "development" && query.archiveDev === "1"} fieldAvailable={currentFieldConfig().mode !== "disabled"} />;
}
