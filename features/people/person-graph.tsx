...
export function PersonGraph({ center, people, relations, onFocus }: { center: Person; people: Person[]; relations: PersonRelation[]; onFocus: (id: string) => void }) {
  ...
  return <div className="hidden h-[31rem] overflow-hidden rounded-2xl border border-ink/10 bg-[linear-gradient(rgba(23,40,36,.04)_1px,transparent_1px),linear-gradient(90deg,rgba(23,40,36,.04)_1px,transparent_1px)] bg-[size:28px_28px] md:block" aria-label="人物一度关系图"><ReactFlow nodes={nodes} edges={edges} fitView minZoom={0.5} maxZoom={1.5} onNodeClick={(_, node) => onFocus(node.id)} proOptions={{ hideAttribution: true }}><Background color="transparent" /><Controls showInteractive={false} /></ReactFlow></div>;
}
