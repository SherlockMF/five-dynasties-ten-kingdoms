...
export function PersonDetailPanel({ person, enableChat = true }: { person: Person; enableChat?: boolean }) {
  return (
    <section aria-labelledby="person-name" className="rounded-2xl bg-ink p-6 text-paper">
      <div className="flex items-center justify-between gap-3"><p className="text-[10px] tracking-[0.18em] text-gold uppercase">Person profile · 人物</p>{enableChat ? <PersonChat person={person} /> : null}</div>
