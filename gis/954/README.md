...
本目录记录 `snapshot-954` 的校勘口径与来源元数据。发布层是根据本地扫描图人工概括的 WGS84 GeoJSON；当前没有 GeoPackage 或 QGIS 工程，后续 GIS 编辑应以新建工程替换，不把发布文件当作高精度编辑源。

## 时间与范围

- 年份区间：`[954, 955)`，不外推到 953 或 955。
- 地图范围：经度 72—136、纬度 18—55。
- 南方层级：南唐、吴越、后蜀、南汉依据 954 年同年图页重建，标为 `reconstructed + reviewed`。
